...
def inWords(number, level=True):
    ...
    if number < 20:
        ...
        word += inWords(number//100, False) + "hundred" + ("and" if number %
                                                           100 != 0 else "") + inWords(number % 100, False)
    elif 1000 <= number < 1000000:
        word += inWords(number//1000, False) + "thousand" + \
            inWords(number % 1000, False)
